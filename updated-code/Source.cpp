...
int main() {
	...
	while (window.isOpen()) {
		...
		for (unsigned int i = 0; i < spring_parts; i++) parts[i]->update_length(tick);

		// Step 4. Increase the simulation time counter
		current_time += tick;

		length sum_length1 = 0;
		for (unsigned int i = 0; i < spring_parts; i++) sum_length1 += parts[i]->part_current;
		// Step 5. Display results after each 10000 iterations
		iteration_counter++;
		if (iteration_counter % 1000 == 0) {
			length sum_length = 0;
			for (unsigned int i = 0; i < spring_parts; i++) sum_length += parts[i]->part_current;
#ifdef VISUALIZE
			...
			window.draw(s_cover);
			s_end.setPosition(sf::Vector2f(window_size_x / 2 - 3 + sum_length / 20 * window_size_x, positionindicator + 3));
			window.draw(s_end);
		...
		if (sum_length1 <= 0.01 && side) { std::cout << current_time - last_time << "\n"; last_time = current_time; side = false; }
		else if (sum_length1 <= 0.001 && (!side)) { std::cout << current_time - last_time << "\n"; last_time = current_time; side = true; }
